...
export const SectionWrapper: React.FC<SectionWrapperProps> = ({ id, children, className = "", hoverDirection, stable = false }) => {
  ...
  return (
    <section id={id} ref={ref} className={`relative py-24 px-6 sm:px-12 lg:px-24 min-h-screen flex flex-col items-center justify-center overflow-hidden ${className}`} style={stable ? undefined : { perspective: '1200px' }}>
      ...
      <motion.div
        ...
        whileHover={
          !stable && hoverDirection === 'left' ? { rotateY: -4, rotateX: 2, scale: 0.98 } :
          !stable && hoverDirection === 'right' ? { rotateY: 4, rotateX: 2, scale: 0.98 } :
          {}
        }
