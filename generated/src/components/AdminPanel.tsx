              ...
              {localData.projects.map(project => (
                <div key={project.id} className="glass-panel p-6 rounded-xl border border-gray-800 relative group">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <label className="md:col-span-2 flex items-center gap-2"><input type="checkbox" checked={project.featured === true} onChange={e => setLocalData({ ...localData, projects: localData.projects.map(p => ({ ...p, featured: p.id === project.id ? e.target.checked : false })) })} />Featured build (one project)</label>
                    {([['videoUrl', 'Demo video URL (MP4/WebM or watch link)'], ['problem', 'Problem'], ['contribution', 'My contribution'], ['result', 'Result — use real evidence only']] as const).map(([field, label]) => <label key={field} className="md:col-span-2 text-sm">{label}<textarea value={project[field] ?? ''} onChange={e => handleProjectChange(project.id, field, e.target.value)} className="mt-2 w-full bg-black/50 border border-gray-700 rounded p-2" rows={2} /></label>)}
                    <div className="md:col-span-2 space-y-3"><h4>Architecture components (ordered flow)</h4>{(project.architecture ?? []).map((node, index) => <div key={index} className="flex flex-wrap gap-2"><input aria-label="Component name" placeholder="Component name" value={node.label} onChange={e => setLocalData({ ...localData, projects: localData.projects.map(p => p.id === project.id ? { ...p, architecture: p.architecture?.map((n, i) => i === index ? { ...n, label: e.target.value } : n) } : p) })} className="bg-black/50 border border-gray-700 rounded p-2 min-w-0" /><input aria-label="Component role" placeholder="What it does" value={node.description} onChange={e => setLocalData({ ...localData, projects: localData.projects.map(p => p.id === project.id ? { ...p, architecture: p.architecture?.map((n, i) => i === index ? { ...n, description: e.target.value } : n) } : p) })} className="bg-black/50 border border-gray-700 rounded p-2 flex-1 min-w-0" /><button type="button" onClick={() => setLocalData({ ...localData, projects: localData.projects.map(p => p.id === project.id ? { ...p, architecture: p.architecture?.filter((_, i) => i !== index) } : p) })}>Remove</button></div>)}<button type="button" className="enhancement-button" onClick={() => setLocalData({ ...localData, projects: localData.projects.map(p => p.id === project.id ? { ...p, architecture: [...(p.architecture ?? []), { label: '', description: '' }] } : p) })}>Add architecture component</button></div>
                    <textarea
